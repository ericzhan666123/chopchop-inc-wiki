...
export function MapPage({ locale, category }: { locale: Locale; category?: MapCategorySlug }) {
  ...
  return <>
    ...
    {category === "trees-stone" && <p><Link href={pathFor(locale, "/guides/stone-tree-location")}>{de ? "Vollständiger Guide →" : "Full guide →"}</Link></p>}
    {category === "rubber-ducks" && <p><Link href={pathFor(locale, "/guides/duck-locations")}>{de ? "Vollständiger Guide →" : "Full guide →"}</Link></p>}
