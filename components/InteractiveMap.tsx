...
export function InteractiveMap({ markers, locale, lockedCategory }: { markers: MapMarker[]; locale: Locale; lockedCategory?: MapCategorySlug }) {
  ...
  const selectMarker = (marker: MapMarker) => setSelected({ name: markerName(marker, locale), x: marker.x, y: marker.y, z: marker.z, elevation: marker.elevationRelativeToCamp });
  ...
  return <section className="map-tool" aria-label={de ? "Interaktive Karte" : "Interactive map"}>
    ...
    <div className="map-scroll" tabIndex={0} aria-label={de ? "Zoombare und verschiebbare Karte" : "Zoomable, pannable map"}>
      <svg className="world-map-svg" style={{ width: `${zoom * 100}%` }} viewBox={`0 0 ${canvasWidth} ${canvasHeight}`} role="img" aria-label={`${layer === "overworld" ? (de ? "Oberwelt" : "Overworld") : "Dungeon"}: ${visible.length} ${de ? "sichtbare Marker" : "visible markers"}`}>
        ...
        {visible.map((marker) => {
          ...
          return <g key={marker.id} className={`map-marker ${category}`} tabIndex={0} role="button" aria-label={markerName(marker, locale)} onMouseEnter={() => selectMarker(marker)} onFocus={() => selectMarker(marker)} onClick={() => selectMarker(marker)}>
            <MarkerIcon marker={marker} x={x} y={y} selected={selected?.name === markerName(marker, locale)}/>
            {category === "landmarks" && !(showPlaces && (marker.subtype === "tent" || marker.subtype === "old-windmill")) && <text className="landmark-label" x={x + 12} y={y + 4}>{markerName(marker, locale)}</text>}
          </g>;
        })}
...
function MarkerIcon({ marker, x, y, selected }: { marker: MapMarker; x: number; y: number; selected: boolean }) {
  const category = markerCategory(marker)!;
  ...
  if (marker.type === "tree") return <g {...common}><path fill={treeColors[marker.subtype ?? ""] ?? "#8fb66a"} d="M0-11 8-2H4l6 7H3v6h-6V5h-7l6-7h-4z"/></g>;
