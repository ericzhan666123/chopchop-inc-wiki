...
export function IndexPage({ kind, locale }: { kind: "recipes" | "items"; locale: Locale }) {
  ...
  const hub=kind==="recipes"?"Database":"Database";
  return <Shell locale={locale}><Breadcrumbs locale={locale} items={[{name:hub,path:"/database"},{name:l[kind]}]}/><div className="eyebrow">Chop Chop Inc.</div><h1>{l[kind]}</h1><p className="muted">{rows.length} {l.count}</p><SearchGrid rows={rows} searchLabel={l.search} filterLabel={l.filter} allLabel={l.all} /><DataSourceNote locale={locale}/></Shell>;
}
...
export function CategoryPage({ kind, category, locale }: { kind: "recipes" | "items"; category: string; locale: Locale }) {
  ...
  return <Shell locale={locale}><Breadcrumbs locale={locale} items={[{name:"Database",path:"/database"},{name:l[kind],path:`/${kind}`},{name:categoryLabel(category,locale)}]}/><h1>{categoryLabel(category, locale)}</h1><p className="muted">{rows.length} {l.count}</p><SearchGrid rows={rows} searchLabel={l.search} filterLabel={l.filter} allLabel={l.all} />{kind==="recipes"&&<DataSourceNote locale={locale}/>}</Shell>;
}
...
export function CollectionPage({ kind, locale }: { kind: "ducks" | "trees" | "audiences" | "npcs"; locale: Locale }) {
  ...
  return <Shell locale={locale}><Breadcrumbs locale={locale} items={[{name:locale==="de"?"Welt":"World",path:"/world"},{name:l[kind]}]}/><div className="eyebrow">Chop Chop Inc.</div><h1>{l[kind]}</h1><p className="muted">{data[kind].length} {l.count}</p><EntryList entries={data[kind]} locale={locale} /></Shell>;
}
